...
inline double mgc_floor(double d) { return 0.0; }
  ...
  inline bool iv_equal(const int *op1, const int *op2){
    const int M1 = MAX(N1,N2);
    const int M2 = MIN(N1,N2);
  ...
  inline void iv_mult(const int *op1, const int *op2, int *r) {
    if(Nr==1)
      ...
      const int *OP1 = N1 >= N2 ? op1 : op2;
      const int *OP2 = N1 >= N2 ? op2 : op1;
      const int T1 = MIN(M2-1,Nr);
      const int T2 = MIN(M1-1,Nr);
      const int T3 = MIN(M1+M2-2,Nr);
  ...
  inline void iv_div(const int *op1, const int *op2, int *r) {
    if(N1==1) {
      if(N2==1) {
        ...
        AC_ASSERT(0, "Divisor > 64-bits, Division currently not implemented beyond 64-bit");
    } 
    else if(N1==2) {
      if(N2==1) {
      ...
      AC_ASSERT(0, "Dividend > 64-bits, Division currently not implemented beyond 64-bit");
  }
  ...
  inline void iv_bitwise_and(const int *op1, const int *op2, int *r) {
    const int M1 = MIN(MAX(N1,N2), Nr);
    const int M2 = MIN(MIN(N1,N2), Nr);
    const int *OP1 = N1 > N2 ? op1 : op2;
    const int *OP2 = N1 > N2 ? op2 : op1;
  ...
  inline void iv_const_shift_l(const int *op1, int *r) {
    ...
    if(!B) {
      ...
      if(s31) {
        for(unsigned i=0; i < MIN(ishift,Nr); i++)
          r[i] = 0;
        unsigned lw = 0;
        const unsigned M1 = MIN(N+ishift,Nr);
  ...
  template<> inline int to_str<AC_DEC>(const int *v, int w, bool left_just, char *r) { 
    ...
    if(w > 32)
      l |= (Ulong) v[1] << 32; 
    if(left_just) {
      AC_ASSERT(w <= 60, "Conversion to decimal affected by current limitation on division"); // otherwise not implemented yet
      l <<= 60-w;
      while(l) {
        l *= 10; 
        r[k++] = (char) ('0' + (int) (l >> 60));
      ...
      AC_ASSERT(w <= 64, "Conversion to decimail affected by current limitation on division"); // otherwise not implemented yet
      while(l) {
        r[k++] = (char) ('0' + (int) (l % 10));
        l /= 10;
      }
...
public:
  ...
  struct rt {
    enum {
      mult_w = W+W2, 
      mult_s = S||S2,
      plus_w = MAX(W+(S2&&!S),W2+(S&&!S2))+1,
      plus_s = S||S2,
      minus_w = MAX(W+(S2&&!S),W2+(S&&!S2))+1,
      minus_s = true,
      div_w = W+S2,
      div_s = S||S2,
      mod_w = MIN(W,W2+(!S2&&S)), 
      mod_s = S,
      logic_w = MAX(W+(S2&&!S),W2+(S&&!S2)),
      logic_s = S||S2
    };
...
// -- C int types -----------------------------------------------------------------
#define SPECIAL_VAL_FOR_INTS(C_TYPE) \
template<ac_special_val val> inline C_TYPE value(C_TYPE); \
template<> inline C_TYPE value<AC_VAL_0>(C_TYPE) { return 0; } \
template<> inline C_TYPE value<AC_VAL_DC>(C_TYPE) { C_TYPE x; return x; }
                                                                                           
SPECIAL_VAL_FOR_INTS(bool);
SPECIAL_VAL_FOR_INTS(char);
SPECIAL_VAL_FOR_INTS(signed char);
SPECIAL_VAL_FOR_INTS(unsigned char);
SPECIAL_VAL_FOR_INTS(signed short);
SPECIAL_VAL_FOR_INTS(unsigned short);
SPECIAL_VAL_FOR_INTS(signed int);
SPECIAL_VAL_FOR_INTS(unsigned int);
SPECIAL_VAL_FOR_INTS(signed long);
SPECIAL_VAL_FOR_INTS(unsigned long);
SPECIAL_VAL_FOR_INTS(signed long long);
SPECIAL_VAL_FOR_INTS(unsigned long long);
